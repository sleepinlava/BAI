...
本文把 ABI 已经具备的控制层检查与尚未完成的科学验证分开记录。只有相应证据齐备后，
才能把一条工作流描述为可复现或经过生物学验证。
...
ABI **目前还不是**经过完整验证的科学工作流。单个工具的论文可以支持组件选择，但整条
路线仍需要固定环境、数据库清单、基准数据集、预期生物学输出和明确的验收阈值。
...
- 添加精心策划的小型阳性对照：已知质粒参考、阴性染色体对照以及混合样本。
  Benchmark harness 与 smoke 测试已存在，但当前 checkout 不包含完整的
  `data/benchmarks/` fixture 树；在宣称可复现 benchmark 前必须配置并冻结准确数据集。
  RefSeq 质粒验证已通过（3/3，genomad+platon majority_vote）。
