...
命令记录写明原因（`resume reuse rejected: …`）。旧结果仍可读取，但缺少
有效恢复身份的运行无法证明复用安全，需要重新执行，不能仅凭文件存在复用。
恢复身份会为外部原始输入记录内容摘要，并排除 DAG 上游步骤已声明的具体产物
路径；单独的 `output_dir` 不代表其下所有文件都归 ABI 产物所有，因此用户提供
的文件和符号链接仍受保护。外部输入发生变化时，恢复仍会拒绝复用。
...
基础报告包含“执行事实”章节——步骤状态计数、失败调用及原因、复用（验证恢
复）步骤、历史关联——使报告不再只是复述计划表。`inspect` 暴露同样的关联
字段（`run_id`、`plan_id`、`resumes_run_id`、`previous_run_archive`）以及
复用步骤列表与审计快照是否存在。
