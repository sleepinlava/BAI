...
Core Python code lives in `src/abi/`. Keep transport-neutral behavior in the core; CLI, MCP, HTTP, and provider integrations should remain thin adapters. Official plugin source is maintained in `sleepinlava/abi-plugin`, pinned as the `src/abi/plugins` Git submodule. Initialize it with `git submodule update --init --recursive`. Plugin wheels are distributed only through GitHub Releases; only `abi-agent` is published to PyPI. Built-in workflow implementations are co-located with their declarative definitions in `src/abi/plugins/<analysis_type>/` (`pipeline_dag.yaml`, tool registries, schemas, report metadata, and a mandatory `limitations.yaml`). Tests are organized under `tests/unit/`, `tests/integration/`, and `tests/smoke/`; local paper reproduction is explicitly selected outside the product suite. Use `examples/` for runnable configuration samples, `docs/en/` and `docs/zh/` for documentation, `envs/` for Conda environments, `environments.yaml` for tool→env assignments (generated environment YAMLs must match the manifest), and `scripts/` for maintenance utilities.
