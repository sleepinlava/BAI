...
whenToUse: When the user asks to run, plan, configure, or inspect a bioinformatics analysis such as 16S amplicon, RNA-seq, WGS bacteria, metagenomics, plasmid, or virome analysis.
license: MIT
...
## Tool naming by platform

- **Kimi Code**: ABI MCP tools are exposed as `mcp__abi__<tool>` (for example `mcp__abi__abi_list_types`).
- **Codex / Claude Code / OpenCode**: ABI MCP tools are exposed as `abi_<tool>` (for example `abi_list_types`).

The lifecycle below uses the ABI tool names; prefix them according to the agent platform you are running in.
...
The default ABI MCP profile is `safe` and does not advertise `abi_run`. If the user wants real execution, explain that the MCP server must be started with `--profile full` (for Kimi Code, change the `args` in `.kimi-code/mcp.json` to `["--profile", "full"]` and reload).
