    ...
    *,
    ...
    if tool_versions:
        for tv in tool_versions:
            version = _format_version_cell(tv.get("version", ""), tv.get("status", ""))
            lines.append(
                f"| {tv.get('tool_id', '')} "
                f"| {tv.get('executable', '')} "
                f"| {version} "
    ...
    lines.extend(
        [
            ...
            "- Tool versions marked as `not_configured` had no version recorded "
            "(the version command was empty or not configured).",
            "- Tool versions marked as `not_captured` could not be captured, "
            "for example because the tool was mocked or an unexpected failure occurred.",
            "- Tool versions marked as `capture_failed (...)` had a version command "
            "but it returned a non-zero exit code or did not match the expected pattern.",
            "- Reference databases marked as `not_configured` were not provided; "
            "those marked as `not_captured` lack recorded version information.",
            "",
